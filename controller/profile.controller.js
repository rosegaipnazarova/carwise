...
const getOneProfile = async (req, res, next) => {
    try {
        const { id } = req.params
        const foundedProfile = await ProfileSchema.findById(id).populate("userId")
        ...
        if (foundedProfile.role === 'admin') {
            cars = await CarSchema.find({ adminId: foundedProfile.userId })
        }

        res.status(200).json({
            profile: foundedProfile,
            cars: cars
        })
    } catch (error) {
        next(error)
